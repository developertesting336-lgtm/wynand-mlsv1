...
const renderMonthlyRentEmail = ({ recipientName, recipientType, renterName, ownerName, listing, booking, amount, currency, paymentDate, paymentForMonthYear }) => {
  const conditions = booking.agreement_conditions || {};
  const propertyName = listing?.title || conditions.propertyAddress || 'Your rental property';
  const isOwner = recipientType === 'owner';
  const intro = isOwner
...
app.post('/send-monthly-rent-invoice', async (req, res) => {
  ...
  const { bookingId, amount, currency = 'MXN', paymentDate, paymentForMonthYear } = req.body;
