...
const renderPaymentEmail = ({ recipientName, recipientType, renterName, ownerName, listing, booking, amount, currency, paymentDate, invoiceId }) => {
    ...
    <div style="margin:0;background:#f3f6f8;padding:32px 16px;font-family:Arial,Helvetica,sans-serif;color:#17212b;">
      <div style="max-width:620px;margin:0 auto;background:#ffffff;border:1px solid #e3e9ee;border-radius:18px;overflow:hidden;box-shadow:0 8px 30px rgba(15,23,42,.07);">
        ...
        <div style="padding:32px 30px 12px;">
          ...
          ${invoiceId ? `<p style="margin: 0; color: #71808c; font-size: 13px;"><strong>Invoice ID:</strong> ${escapeHtml(invoiceId)}</p>` : ''}
          <p style="margin:16px 0 0;color:#5d6b78;font-size:15px;line-height:1.7;">Hi ${escapeHtml(recipientName || (isOwner ? ownerName : renterName) || 'there')},</p>
...
const renderMonthlyRentEmail = ({ recipientName, recipientType, renterName, ownerName, listing, booking, amount, currency, paymentDate, paymentForMonthYear, invoiceId }) => {
  const conditions = booking.agreement_conditions || {};
  const propertyName = listing?.title || conditions.propertyAddress || 'Your rental property';
  const isOwner = recipientType === 'owner';
  const intro = isOwner
    ? `Great news! <strong>${escapeHtml(renterName)}</strong> has paid the monthly rent for <strong>${escapeHtml(propertyName)}</strong>.`
    : `Your monthly rent payment for <strong>${escapeHtml(propertyName)}</strong> was received successfully.`;
  ...
  return `
    <div style="margin:0;background:#f3f6f8;padding:32px 16px;font-family:Arial,Helvetica,sans-serif;color:#17212b;">
      <div style="max-width:620px;margin:0 auto;background:#ffffff;border:1px solid #e3e9ee;border-radius:18px;overflow:hidden;box-shadow:0 8px 30px rgba(15,23,42,.07);">
        ...
        <div style="padding:32px 30px 12px;">
          <div style="display:inline-block;padding:7px 11px;border-radius:999px;background:#e8f8f0;color:#11804b;font-size:12px;font-weight:bold;letter-spacing:.04em;text-transform:uppercase;">Rent Paid</div>
          <h1 style="margin:16px 0 10px;font-size:26px;line-height:1.2;color:#17212b;">${isOwner ? 'Rent Payment Received' : 'Monthly Rent Receipt'}</h1>
          ${invoiceId ? `<p style="margin: 0; color: #71808c; font-size: 13px;"><strong>Invoice ID:</strong> ${escapeHtml(invoiceId)}</p>` : ''}
          <p style="margin:16px 0 0;color:#5d6b78;font-size:15px;line-height:1.7;">Hi ${escapeHtml(recipientName || (isOwner ? ownerName : renterName) || 'there')},</p>
          <p style="margin:8px 0 0;color:#5d6b78;font-size:15px;line-height:1.7;">${intro}</p>
        </div>
        <div style="margin:22px 30px 0;padding:20px;border:1px solid #e6edf2;border-radius:14px;background:#fbfcfd;">
          <p style="margin:0 0 15px;font-size:12px;font-weight:bold;letter-spacing:.08em;text-transform:uppercase;color:#73808c;">Payment & Rent Details</p>
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="font-size:14px;line-height:1.6;">
            <tr><td style="padding:6px 0;color:#71808c;">Property</td><td style="padding:6px 0;text-align:right;font-weight:bold;color:#17212b;">${escapeHtml(propertyName)}</td></tr>
            <tr><td style="padding:6px 0;color:#71808c;">Tenant</td><td style="padding:6px 0;text-align:right;color:#17212b;">${escapeHtml(renterName || '—')}</td></tr>
            <tr><td style="padding:6px 0;color:#71808c;">Owner</td><td style="padding:6px 0;text-align:right;color:#17212b;">${escapeHtml(ownerName || '—')}</td></tr>
            <tr><td style="padding:6px 0;color:#71808c;">Rent Period Paid</td><td style="padding:6px 0;text-align:right;font-weight:bold;color:#1296d4;">${escapeHtml(paymentForMonthYear)}</td></tr>
            <tr><td style="padding:12px 0 5px;border-top:1px solid #e6edf2;color:#71808c;">Amount Paid</td><td style="padding:12px 0 5px;border-top:1px solid #e6edf2;text-align:right;font-size:18px;font-weight:bold;color:#11804b;">${escapeHtml(formatMoney(amount, currency))}</td></tr>
            <tr><td style="padding:5px 0;color:#71808c;">Payment date</td><td style="padding:5px 0;text-align:right;color:#17212b;">${escapeHtml(formatDate(paymentDate))}</td></tr>
          </table>
        </div>
...
app.post('/send-payment-invoice', async (req, res) => {
  ...
  const { bookingId, amount, currency = 'MXN', paymentDate, stripeSessionId, stripePaymentIntentId, invoiceId } = req.body;
  ...
  try {
    ...
    const [renterResult, ownerResult] = await Promise.all([
      transporter.sendMail({
        ...
        subject,
        html: renterEmail,
        headers: { 'X-Booking-Id': bookingId, 'X-Stripe-Session-Id': stripeSessionId || '', 'X-Invoice-Id': invoiceId || '' },
      }),
      transporter.sendMail({
        from: `PV Verified Rentals <${process.env.APP_EMAIL}>`,
        to: owner.email,
        subject,
        html: ownerEmail,
        headers: { 'X-Booking-Id': bookingId, 'X-Stripe-Payment-Intent-Id': stripePaymentIntentId || '', 'X-Invoice-Id': invoiceId || '' },
      }),
    ]);
...
// Send monthly rent payment confirmation to both the renter and owner.
app.post('/send-monthly-rent-invoice', async (req, res) => {
  // No internal secret validation needed for monthly rent email endpoint

  const { bookingId, amount, currency = 'MXN', paymentDate, paymentForMonthYear, invoiceId } = req.body;
  if (!bookingId || amount === undefined || amount === null || !paymentForMonthYear) {
    return res.status(400).json({ error: 'bookingId, amount, and paymentForMonthYear are required' });
  }
  ...
  try {
    ...
    const subject = `Monthly Rent Paid: ${listing?.title || 'Rental booking'}`;
    const renterEmail = renderMonthlyRentEmail({ ...details, recipientName: renter.full_name || renter.email, recipientType: 'renter' });
    const ownerEmail = renderMonthlyRentEmail({ ...details, recipientName: owner.full_name || owner.email, recipientType: 'owner' });
    ...
    const [renterResult, ownerResult] = await Promise.all([
      transporter.sendMail({
        ...
        subject,
        html: renterEmail,
        headers: { 'X-Booking-Id': bookingId, 'X-Invoice-Id': invoiceId || '' },
      }),
      transporter.sendMail({
        from: `PV Verified Rentals <${process.env.APP_EMAIL}>`,
        to: owner.email,
    ...
    await logServerAction({ action: 'SEND_MONTHLY_RENT_INVOICE', entityType: 'bookings', entityId: bookingId });
    return res.json({ success: true, recipients: [renter.email, owner.email], messageIds: [renterResult.messageId, ownerResult.messageId] });
  } catch (err) {
    console.error('[send-monthly-rent-invoice] Error:', err);
    return res.status(500).json({ error: err.message || 'Failed to send monthly rent invoice' });
  }
});


// Template for monthly rent due/overdue reminders
const renderRentReminderEmail = ({ recipientName, recipientType, renterName, ownerName, listing, booking, amount, currency, dueDate, paymentForMonthYear, diffDays }) => {
  const conditions = booking.agreement_conditions || {};
  const propertyName = listing?.title || conditions.propertyAddress || 'Your rental property';
  
  let badgeText = 'Rent Reminder';
  let badgeBg = '#fff4e5';
  let badgeColor = '#b45309';
  let titleText = 'Rent Payment Reminder';
  let intro = '';

  if (diffDays === 1) {
    badgeText = 'Rent Due Soon';
    titleText = 'Rent Payment Reminder (Due tomorrow)';
    intro = `This is a friendly reminder that your monthly rent payment of <strong>${formatMoney(amount, currency)}</strong> for <strong>${escapeHtml(propertyName)}</strong> is due tomorrow on ${formatDate(dueDate)}.`;
  } else if (diffDays === -1) {
    badgeText = 'Overdue';
    badgeBg = '#fee2e2';
    badgeColor = '#991b1b';
    titleText = 'Rent Payment OVERDUE';
    intro = `<strong>URGENT:</strong> Your monthly rent payment of <strong>${formatMoney(amount, currency)}</strong> for <strong>${escapeHtml(propertyName)}</strong> was due yesterday on ${formatDate(dueDate)} and is now overdue by 1 day.`;
  } else {
    // general fallback
    badgeText = 'Rent Reminder';
    titleText = 'Rent Payment Reminder';
    intro = `This is a reminder that your monthly rent payment of <strong>${formatMoney(amount, currency)}</strong> for <strong>${escapeHtml(propertyName)}</strong> is due/overdue on ${formatDate(dueDate)}.`;
  }
  ...
  return `
    <div style="margin:0;background:#f3f6f8;padding:32px 16px;font-family:Arial,Helvetica,sans-serif;color:#17212b;">
      <div style="max-width:620px;margin:0 auto;background:#ffffff;border:1px solid #e3e9ee;border-radius:18px;overflow:hidden;box-shadow:0 8px 30px rgba(15,23,42,.07);">
        ...
        <div style="padding:32px 30px 12px;">
          <div style="display:inline-block;padding:7px 11px;border-radius:999px;background:${badgeBg};color:${badgeColor};font-size:12px;font-weight:bold;letter-spacing:.04em;text-transform:uppercase;">${badgeText}</div>
          <h1 style="margin:16px 0 10px;font-size:24px;line-height:1.2;color:#17212b;">${titleText}</h1>
          <p style="margin:0;color:#5d6b78;font-size:15px;line-height:1.7;">Hi ${escapeHtml(recipientName || renterName || 'there')},</p>
          <p style="margin:8px 0 0;color:#5d6b78;font-size:15px;line-height:1.7;">${intro}</p>
        </div>
        <div style="margin:22px 30px 0;padding:20px;border:1px solid #e6edf2;border-radius:14px;background:#fbfcfd;">
          <p style="margin:0 0 15px;font-size:12px;font-weight:bold;letter-spacing:.08em;text-transform:uppercase;color:#73808c;">Outstanding Details</p>
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="font-size:14px;line-height:1.6;">
            <tr><td style="padding:6px 0;color:#71808c;">Property</td><td style="padding:6px 0;text-align:right;font-weight:bold;color:#17212b;">${escapeHtml(propertyName)}</td></tr>
            <tr><td style="padding:6px 0;color:#71808c;">Tenant</td><td style="padding:6px 0;text-align:right;color:#17212b;">${escapeHtml(renterName || '—')}</td></tr>
            <tr><td style="padding:6px 0;color:#71808c;">Amount Due</td><td style="padding:6px 0;text-align:right;font-weight:bold;color:#991b1b;">${escapeHtml(formatMoney(amount, currency))}</td></tr>
            <tr><td style="padding:6px 0;color:#71808c;">Due Date</td><td style="padding:6px 0;text-align:right;font-weight:bold;color:#17212b;">${escapeHtml(formatDate(dueDate))}</td></tr>
            <tr><td style="padding:6px 0;color:#71808c;">Billing Period</td><td style="padding:6px 0;text-align:right;color:#1296d4;">${escapeHtml(paymentForMonthYear)}</td></tr>
          </table>
        </div>
        <div style="padding:25px 30px 32px;text-align:center;">
          <p style="margin:24px 0 0;color:#9aa6b1;font-size:12px;line-height:1.6;">PV Verified Rentals<br />Thank you for using our secure rental platform.</p>
...
// Send monthly rent reminder to renter
app.post('/send-rent-reminder', async (req, res) => {
  const { bookingId, amount, currency = 'MXN', dueDate, paymentForMonthYear, diffDays } = req.body;
  if (!bookingId || amount === undefined || amount === null || !paymentForMonthYear || !dueDate || diffDays === undefined) {
    return res.status(400).json({ error: 'bookingId, amount, paymentForMonthYear, dueDate, and diffDays are required' });
  }

  try {
    const { data: booking, error: bookingError } = await supabase
      .from('bookings')
      .select('id, owner_id, renter_id, listing_id, agreement_conditions')
      .eq('id', bookingId)
      .single();
    if (bookingError || !booking) throw new Error('Booking not found');
    ...
    if (!renter?.email) throw new Error('Renter email not found');

    const details = {
      recipientName: renter.full_name || renter.email,
    ...
    const subject = `Rent Payment Reminder: ${listing?.title || 'Rental property'}`;
    const emailHtml = renderRentReminderEmail(details);
    ...
    await logServerAction({ action: 'SEND_RENT_REMINDER_EMAIL', entityType: 'bookings', entityId: bookingId });
    return res.json({ success: true, recipient: renter.email, messageId: emailResult.messageId });
  } catch (err) {
    console.error('[send-rent-reminder] Error:', err);
    return res.status(500).json({ error: err.message || 'Failed to send rent reminder email' });
  }
});
